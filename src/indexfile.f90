            ...
            do J = I+1, NGRPS
                if (I .ne. J) then
                    if (this%group(I)%title .eq. this%group(J)%title) then
                        write(error_unit,*)
                        write(error_unit,'(a, a, a)') "dcdfort >> WARNING: Index group ", this%group(I)%title, &
    ...
    function indexfile_get(this, group_name, I)
        ...
        if (.not. present(I)) then
            if (this%group_warning) then
                write(error_unit, '(a)') "dcdfort >> WARNING: No atoms found in index group '"//trim(group_name)//"'."
                write(error_unit, '(a)') "dcdfort >> This warning will not appear again for any other index groups."
                this%group_warning = .false.
            end if
            indexfile_get = 0
        else
            indexfile_get = -1
            write(error_unit, '(a)') "dcdfort >> ERROR: "//trim(group_name)//" is not in index file. The groups available are:"
            do J = 1, size(this%group)
